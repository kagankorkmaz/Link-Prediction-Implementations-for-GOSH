...
template <class T>
float *CSR<T>::get_ra_indexes()
{
    float *ra_indexes = new float[num_edges];
    ...
    { //for each vertex
        ...
        { //for each vertex edge
            ...
            { //already calculated rais
                for (unsigned int k = V[nbr]; k < V[nbr + 1]; k++)
                {
                    if (E[k] == i)
                    {
                        ra_indexes[j] = ra_indexes[k];
                    }
                }
            }
        }
...
template <class T>
T CSR<T>::get_ra_matching(T v, T size, float *ra_indexes, T *match)
{
    T best = -1;
    float best_val = 0;
    for (T i = 0; i < size; i++)
    {
        T edge = V[v] + i;
        if (ra_indexes[edge] > best_val && match[E[edge]] == -1)
        {
            best = E[edge];
            best_val = ra_indexes[edge];
        }
    }
    return best;
}
...
// adamic/adar index
template <class T>
float *CSR<T>::get_aa_indexes()
{
    float *aa_indexes = new float[num_edges];
    ...
    { //for each vertex
        ...
        { //for each vertex edge
            ...
            { //already calculated aais
                for (unsigned int k = V[nbr]; k < V[nbr + 1]; k++)
                {
                    if (E[k] == i)
                    {
                        aa_indexes[j] = aa_indexes[k];
                    }
                }
            }
        }
...
template <class T>
T CSR<T>::get_aa_matching(T v, T size, float *aa_indexes, T *match)
{
    T best = -1;
    float best_val = 0;
    for (T i = 0; i < size; i++)
    {
        T edge = V[v] + i;
        if (aa_indexes[edge] > best_val && match[E[edge]] == -1)
        {
            best = E[edge];
            best_val = aa_indexes[edge];
        }
    }
    return best;
}